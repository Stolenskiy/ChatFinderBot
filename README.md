...
- віддаються сторінками по `PAGE_SIZE` елементів;
...
- результати в пам'яті автоматично очищаються після TTL;
...
- `PAGE_SIZE` - скільки результатів показувати на одній сторінці
- `PAGE_STATE_TTL_MINUTES` - скільки хвилин тримати в пам'яті результати останнього пошуку користувача для `/nextpage`
...
Показує наступні `PAGE_SIZE` елементів останнього пошуку цього користувача.
